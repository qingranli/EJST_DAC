# plot post-lasso logistic regression results (DAC dummy as dep var.)
...
regfitList = readRDS(paste0("regfit_postLasso_",method,".RData"))
...
# fwrite(regtable, paste0("RegTable_",method,"_save.csv"))
...
p1 = ggplot(dt1[1:10]) +
  ...
  # scale_y_continuous(limits = c(0, 1.08*max(dt1$abs_est)),
  #                    breaks = seq(0, 1.1*max(dt1$abs_est), 
  #                                 round(1.1*max(dt1$abs_est)/10))) +
...
p2 = ggplot(dt2[1:10]) +
  ...
  # scale_y_continuous(limits = c(0, 1.08*max(dt2$abs_est)),
  #                    breaks = seq(0, 1.1*max(dt2$abs_est), 
  #                                 round(1.1*max(dt2$abs_est)/10,2))) +
...
p3 = ggplot(dt3[1:10]) + 
  ...
  # scale_y_continuous(limits = c(0, 1.08*max(dt3$abs_est)),
  #                    breaks = seq(0, 1.1*max(dt3$abs_est), 
  #                                 round(1.1*max(dt3$abs_est)/7,2))) +
...
p4 = ggplot(dt4[1:10]) + 
  ...
  # scale_y_continuous(limits = c(0, 1.3*max(dt4$abs_est)),
  #                    breaks = seq(0, 1.3*max(dt4$abs_est), 
  #                                 round(1.3*max(dt4$abs_est)/10,2))) +
...
ggarrange(p1, p2, p3, p4, nrow = 4, align = "v",
          labels = c("a.", "b.", "c.", "d."))
# ggsave(paste0("Fig4_DAC_reg_",method,".jpg"), 
