# Create main Figure 3: post-lasso linear regression results
# Create Supp. Figure 4 (select lambda using min-error method)
# author: Qingran Li
...
method = "minE" # 1SE (for Fig. 3) vs. minE (for Supp Fig. 4)
...
# combine all regression tables (DOE, DOT, NYS) =====================
regtable = rbind(regtable_doe, regtable_dot, regtable_nys)
# fwrite(regtable, paste0("Figure_save/RegTable_score_",method,"_save.csv"))
...
if (method == "1SE") {
  ggsave(paste0("Figure_save/main_Figure 3.pdf"), 
         dpi = 300, width = 8.5, height = 6.75)
}

if (method == "minE") {
  ggsave(paste0("Figure_save/SI_Fig 4.jpg"), 
         dpi = 300, width = 8.5, height = 6.75)
}
